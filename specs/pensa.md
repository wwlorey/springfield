...
- **`.pensa/daemon.url`** — written by ralph before sandbox launch, contains `http://localhost:<port>`. Tells sandbox-side `pn` to connect to the host daemon via proxy instead of auto-starting locally. Cleaned up by ralph on exit.
