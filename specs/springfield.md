...
`sgf` handles all prompt templating for automated stages before invoking ralph. Ralph is template-unaware — it receives a final assembled prompt. Interactive stages do not go through `sgf` prompt assembly — the wrapper handles system prompt injection.
...
Interactive stages (`spec`, `issues log`) call `$AGENT_CMD` directly. No PID file, no log tee, no loop ID. The wrapper (configured by the user) handles system prompt injection. Automated stages (`build`, `verify`, `test-plan`, `test`) go through ralph and always run in Docker sandboxes.
