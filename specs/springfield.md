...
MEMENTO.md                             (lookup reference document; loaded first automatically into each agent's context window)
...
`sgf init` is safe to re-run. It skips files that already exist (prompts, MEMENTO.md, CLAUDE.md, specs/README.md) and only merges additive content (deny rules, git hooks, gitignore entries). It never overwrites existing content.
...
1. **Orient** — `MEMENTO.md` is already in context (prepended by sgf). Follow its `study` directives.
