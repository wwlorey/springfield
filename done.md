...
- [x] **Factor out loop boilerplate** — Defined "Standard Loop Iteration" pattern (7 steps) shared by Build and Test. Issues Plan has its own explicit iteration flow (bugs excluded from `pn ready`). Per-stage deltas in a table. Dropped explicit `pn export` from agent steps — pre-commit hook handles it.
...
- [x] **Add atomic `pn claim-next` command** — Resolved differently: no new command. Following beads' pattern, `pn update --claim` is the atomic operation (`UPDATE ... WHERE status = 'open'`, fails with `already_claimed` if another agent got there first). Agent keeps choice — queries `pn ready`, picks a task, attempts claim, re-queries on conflict. Standard Loop Iteration stays 7 steps with explicit Query → Choose & Claim separation. Scoped the standard pattern to Build and Test only — Issues Plan gets its own explicit iteration flow since bugs are excluded from `pn ready`.
