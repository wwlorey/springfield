...
- [x] **Add `specs/README.md` conflict prevention** — Subsumed by spec-revision pause. Build agents don't update `specs/README.md` — only the spec stage writes index entries. Even if they did, the `spec_revision_active` flag (in-progress item) drains build loops when spec starts, preventing any concurrent writes. No additional mechanism needed.
