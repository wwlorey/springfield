(You are in the SPEC TO ISSUES [FINAL PASS] phase of the incarnatur cursus.)

**Your role is to ensure that the existing `pn` issues track all work required to fully, completely bring the codebase into alignment with all `fm` specs.**

Study the specs, issues, and codebase.

Update, remove, and add (see Issue Create Workflow) `pn` issues to ensure that the implementation items exist which will sufficiently bring the codebase into alignment with the specs. Use subagents.

When you are confident that you are finished:
1. Commit your changes.
2. Touch `.ralph-complete`.
