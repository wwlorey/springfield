...
| [shutdown](specs/shutdown.md) | `crates/shutdown/` | stable | Shared graceful shutdown — double-press Ctrl+C/Ctrl+D detection with confirmation prompts |
| [springfield](specs/springfield.md) | `crates/springfield/` | stable | CLI entry point — scaffolding, prompt delivery, iteration runner, loop orchestration, recovery, and daemon lifecycle |
| [test-harness](specs/test-harness.md) | `crates/springfield/tests/` | stable | Cross-crate integration test harness — concurrency control, process lifecycle guards, mock infrastructure, and environment isolation |
